...
| `inspect_page` | Opens caller URL and returns DOM, form, locator, accessibility, and page URL data to the agent. It may load a caller-selected workspace Playwright storage-state file into the throwaway context. | Apply the same target/redirect policy, confine state-file reads to the workspace, never return the state or credential values, and warn that authenticated page content may be sensitive. |
...
| --- | --- | --- | --- | --- | --- | --- |
| T1 | Prompt/page induces arbitrary local code execution. | `run_playwright_test` is explicitly dangerous. Default mode requires an accepted MCP form elicitation showing the exact test digest and effects. The operator-only `--unattended` startup flag replaces per-run elicitation and cannot be set by a tool request or environment variable. Both modes snapshot the authorized direct source, run in a bounded child process, and retain process-group cleanup. Tool annotation and mode-specific descriptions disclose execution. | `test/generated-output-and-tools.test.cjs`: unsupported/declined default clients do not launch; accepted approval produces a digest-bound record; explicit unattended mode runs without elicitation and emits a labeled record; cancellation kills descendants. | QUA-1730 | V, N, R | In unattended mode, any client able to call the server can execute supplied code without per-run review. That code has the operating-system permissions and network reach of the local user, subject only to the documented child environment and runner controls. |
...
| T11 | An authenticated inspection reads an unrelated local credential file, returns its state, or broadens network access. | `storageStatePath` is workspace-relative, resolves through symlinks inside the workspace, names a bounded regular file, and is used only to seed a throwaway context. The response never includes the path or state. Existing target, redirect, subresource, and WebSocket policy is unchanged. | `test/generated-output-and-tools.test.cjs`: authenticated fixture succeeds only with state; absolute paths, traversal, and symlink escape fail; returned data excludes the fixture state. | #53 | V, N, R | The authenticated page content returned for inspection may itself contain private data, and the selected site receives matching credentials by design. |
...
| `inspect_page` | Caller URL, DOM/accessibility/forms/locators, and optional workspace-relative Playwright storage state | Reads selected state file; no intended write | Supplied public URL plus page resources | Must declare network and credential-state use; state remains local and results are treated as untrusted, potentially private page data and redacted/bounded. |
