...
The following is source review at commit `5b34d7be383a07237a3a40a8477f0029d317e570`.
...
| `proxy` | Runs only when explicitly invoked with an API key, forwards stdio JSON-RPC to a configurable URL, and forwards the hosted response to stdout. | Pin/validate endpoint and redirect behavior, prevent token/error leakage, bound response handling, and document the exact hosted data boundary. |
...
| --- | --- | --- | --- | --- | --- | --- |
| T1 | Prompt/page induces arbitrary local code execution. | `run_playwright_test` is explicitly dangerous, requires a per-call execution acknowledgement, accepts a single approved-workspace test path or inline test only, runs it in a restricted child sandbox, and has process-group cleanup. Tool annotation discloses execution. | `security/run-playwright-execution-consent.test.ts`: no child launch without acknowledgement; outside-workspace path is rejected; one selected test only; cancellation kills descendants. | QUA-1727 | V, N, R | User-approved test code can still act within the documented sandbox allowance. |
| T2 | Inline/test-file code reads or exfiltrates inherited credentials. | Start child environment from a documented minimal allowlist; never merge parent environment. Explicit per-call allowlisted variables are non-secret by policy. | `security/run-playwright-env-isolation.test.ts`: sentinel parent variable absent; allowlisted `BASE_URL` present; no returned token-shaped value. | QUA-1727 | V, N, R | Code can access credentials deliberately placed in its allowed sandbox; users must not opt in secrets. |
| T3 | Tool output, browser console, child stderr, `--clients`, or proxy errors disclose a secret. | Central redaction for recognized credential forms before every MCP/CLI/artifact return; size caps; never echo request headers, environment, or raw hosted errors; `--clients` must always render a literal placeholder instead of an environment-sourced key. | `security/output-redaction.test.ts`: seeded token-shaped strings in console, stdout, stderr, `--clients`, and transport error are redacted or absent at source. | QUA-1727 | V, N, R | Unknown secret formats may evade pattern redaction; minimize capture and expose a disclosure process. |
| T4 | Public target redirects or links into loopback, RFC1918, CGNAT, multicast, unspecified, IPv6 local, link-local, or cloud metadata space. | Default-deny special/private ranges; metadata and link-local always denied; per-call localhost consent only for an exact loopback target/port; validate initial URL, every redirect, every browser request, and every discovered link. | `security/network-policy.test.ts`: IPv4/IPv6/textual aliases, metadata endpoints, redirect chain, browser subresource request, and page links are denied; explicit localhost case succeeds only under consent. | QUA-1728 | V, N, R | A user who expressly consents to a local target accepts that target's local exposure. |
| T5 | DNS changes after validation (rebinding). | Resolve immediately before connection, reject disallowed resolved addresses, connect to the validated address with the intended Host/SNI, and repeat validation on redirects. | `security/dns-rebinding.test.ts`: injected resolver changes public answer to private answer; connection is refused and audit reason is returned without address details. | QUA-1728 | V, N, R | DNS/network semantics differ by platform; supported resolver behavior must be documented and retested. |
| T6 | Generated path traverses outside approved output or follows a symlink; an existing file is overwritten. | Canonicalize output root and candidate path; reject absolute/traversal/symlink escape; create new files atomically; require explicit overwrite confirmation. | `security/generated-file-boundary.test.ts`: traversal, absolute path, symlink, collision, and approved nested path cases. | QUA-1729 | V, N, R | Files inside the explicit approved root remain user-controlled data. |
| T7 | Child Playwright process hangs, forks, or survives cancellation. | Enforce wall-clock deadline below tool limit; run in a process group/job; terminate then forcibly reap descendants; bound stdout/stderr and artifacts. | `security/run-playwright-lifecycle.test.ts`: hanging process and spawned descendant are gone after timeout/cancel; response is bounded. | QUA-1727 | V, N, R | OS-specific process-tree semantics require macOS/Linux/Windows coverage before broad support claims. |
...
| `run_playwright_test` | Test path or inline code and allowed base URL | Executes code; creates config/artifacts | Only policy-allowed test navigation | Must declare code execution, filesystem write, and network; explicit execution acknowledgement is mandatory. |
